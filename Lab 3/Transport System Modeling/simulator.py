...
class Simulator:
        ...
        for i in range(len(Graph)):
            ...
            if len(Graph[i][1]) > 0:
                for j in Graph[i][1]:
                    ...
                    self.createRoad((start[0] - offset * sin, start[1] + offset * cos), (end[0] - offset * sin, end[1] + offset * cos), startCross, endCross)
    ...
    def update(self):
        ...
        for gen in self.vehicleGens:
            gen.update()
            if (self.t >= 480 and self.t <= 540) or (self.t >= 1020 and self.t <= 1080):
                gen.vehicleRate = 150
            else:
                gen.vehicleRate = 60
        self.t += self.dt
